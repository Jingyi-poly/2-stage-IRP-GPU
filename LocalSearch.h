...
#include "Noeud.h"
#include "Route.h"
#include "OUPolicySolver.h" // 包含OUPolicySolver.h
...
*/
  ...
  std::vector<DeliveryPlan> vec_delivery_plans_;

  ~LocalSearch(void);

  void PreComputeDeliveryPlan(uint64_t c);
  bool ApplyDeliveryPlan(const int c, const double objective);
  bool EvaluateDeliveryPlan(const int c, const double objective);
};
