...
int LocalSearch::mutationDifferentDayprint()
{
  ...
  int nbMoves = 0;
    ...
    for (int posU = 0; posU < params->nbClients; posU++)
      nbMoves += mutation13(ordreParcours[0][posU]);//random每一个client
...
int LocalSearch::mutationDifferentDay()
{
  ...
  while(!rechercheTerminee){
    ...
    for (int posU = 0; posU < params->nbClients; posU++){
      times++;
      nbMoves += mutation13(ordreParcours[0][posU]);// 为了使用OU Policy，把mutation11改成mutation12
    }
...
// Matrix Policy
int LocalSearch::mutation13(int client) {
  bool trace = false, traces = false;
  ...
  // 使用OUPolicySolver 对子问题进行求解
  MatrixSolver matrixSolver(params, client, noeudTravails); 
  Solution result = matrixSolver.solve();
  // 获取结果
  quantities = result.quantities;
  objective = result.totalCost; 
  vector<Noeud*> places = result.places;

  // // 将quantities[i-1]赋值给quantities[i]
  // for (int i = 1; i < params->nbDays; i++) {
  ...
  if(traces) cout <<tmpCost<<endl;
  if (fabs(tmpCost- objective)>0.01  ) {
    cout << "tmpCost: " << tmpCost << " objective: " << objective << endl;
    cin.get();
    // cout << "tmpCost- objective>0.01" << endl;
    return 0;
  }
  ...
  {
    // cout << "currentCost: " << currentCost << " objective: " << objective << endl;
    // cin.get();
    return 1;
  }
}
...
double LocalSearch::evaluateCurrentCost_stockout(int client)
{
  ...
  for (int k = 1; k <= params->ancienNbDays; k++)
  {
    ...
    if (noeudClient->estPresent){
      // adding the inventory cost库存成本:根据公式计算库存成本并累加到myCost。
        if(trace) cout <<"holding cost: "<<params->cli[client].inventoryCost * (I+demandPerDay[k][client]-params->cli[client].dailyDemand[k])<<" q="<<demandPerDay[k][client]<<" demand = "<<params->cli[client].dailyDemand[k]<<endl;
        myCost +=
