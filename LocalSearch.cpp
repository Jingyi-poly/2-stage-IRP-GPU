...
int LocalSearch::mutationDifferentDayprint()
{
  ...
  int nbMoves = 0;
    ...
    for (int posU = 0; posU < params->nbClients; posU++)
      nbMoves += mutation12(ordreParcours[0][posU]);//random每一个client
...
int LocalSearch::mutationDifferentDay()
{
  ...
  while(!rechercheTerminee){
    ...
    for (int posU = 0; posU < params->nbClients; posU++){
      times++;
      nbMoves += mutation12(ordreParcours[0][posU]);// 为了使用OU Policy，把mutation11改成mutation12
    }
...
int LocalSearch::mutation12(int client) {
  ...
  // 相比较mutation11 使用noeudTravails储存每一天的noeudTravail，便于后续调用
  Noeud *noeudTravail;
  ...
  // insertion和quantity的初始化与mutation11相同，places是OUPolicySolver的输出，放在之后定义
  vector<vector<Insertion>> insertions = vector<vector<Insertion>>(params->nbDays);
  vector<double> quantities = vector<double>(params->nbDays);
  ...
  // 使用OUPolicySolver 对子问题进行求解
  OUPolicySolver ouPolicySolver(params, clients[1][client], client, noeudTravails); 
  ...
  for (int k = 1; k <= params->ancienNbDays; k++)
  { 
    if (quantities[k - 1] > 0.0001 && places[k - 1] != nullptr) // 直接使用OUPolicySolver的结果places，不需要原先的breakpoint
    {
      demandPerDay[k][client] = round(quantities[k - 1]);
  ...
  if(traces) cout <<tmpCost<<endl;
  if (fabs(tmpCost- objective)>0.01  ) {
    cout << "tmpCost- objective>0.01" << endl;
    return 0;
  }
  if ( currentCost-objective >=0.01 )
  {
    rechercheTerminee = false;
...
// constructeur
LocalSearch::LocalSearch(void) : params(nullptr), individu(nullptr) {
  // 初始化成员变量以避免在析构函数中访问未初始化的内存
}
...
{
  // 检查params是否为空，如果为空说明这是默认构造函数创建的对象，无需清理
  if (params != nullptr) {
    // 安全清理clients
    try {
      if (!clients.empty()) {
...
void LocalSearch::PreComputeDeliveryPlan(uint64_t c) {
  ...
  vector<Noeud*> nodes;
  nodes.clear();
  nodes.push_back(nullptr); // 控掉index=0
  double best_load = 0.;
  Noeud* best_place = nullptr;
  bool force_delivery = false;
  vector<int> vec_force_delivery_clients = {2,5,3};
  vec_delivery_plans_.clear();
  // 修复：将大小设置为nbDays + 1，与daily_plans的大小匹配
...
bool LocalSearch::ApplyDeliveryPlan(const int c, const double objective) {
  ...
  // 打印 vec_delivery_plans_ 的 choose_delivery 和 insert_place 和 quantity
  if (traces) {
  ...
  { 
    ...
    if (vec_delivery_plans_[k - 1].choose_delivery) // 直接使用OUPolicySolver的结果places，不需要原先的breakpoint
    {
      // 添加空指针检查
...
bool LocalSearch::EvaluateDeliveryPlan(const int c, const double objective) {
  ...
  if (fabs(tmpCost- objective)>0.01) throw std::runtime_error("tmpCost- objective>0.01: " + std::to_string(tmpCost) + " " + std::to_string(objective));
  return current_cost_-objective >=0.01;
}
