...
// void Params::init_scenario_2_daily_demand() {
// 	bool trace = true;
// 	if (nb_scenario_ == 0) {
// 		nb_scenario_ = 1;
// 	}
...
// 	vec_client_scenario_daily_demand_.clear();
// 	vec_client_scenario_daily_demand_.resize(nbClients);
// 	for (int c = 0; c < nbClients; c++) {
// 		vec_client_scenario_daily_demand_[c].resize(nb_scenario_);
// 	}
	
// 	for (int c = 0; c < nbClients; c++) {
// 		// 获取每一个client的maxInventory 作为基数
// 		double max_inventory = cli[c+nbDepots].maxInventory;
// 		for (uint64_t s = 0; s < nb_scenario_; ++s) {
// 			vec_client_scenario_daily_demand_[c][s].resize(nbDays + 1, 0.0); // +1 因为天数从1开始索引
...
// 					random_value = 2.0;
// 				}
// 				double demand_rate = 0.4;
// 				demand_rate += random_value * 0.1;
// 				// 根据max_inventory 和 demand_rate 计算出需求 并取整数
// 				vec_client_scenario_daily_demand_[c][s][day] = round(max_inventory * demand_rate);
// 			}
// 		}
// 	}
...
// 		for (int c = 0; c < nbClients; c++) {
// 			cout << "client " << c << " 最大库存:" << cli[c+nbDepots].maxInventory << " 最多前5个场景的每天的需求:" << endl;
// 			for (int s = 0; s < nb_scenario_; s++) {
// 				cout << "场景 " << s << " 每天的需求:";
// 				for (int day = 1; day <= nbDays; day++) {
// 					cout << vec_client_scenario_daily_demand_[c][s][day] << " ";
// 				}
// 				cout << endl;
// 				if (s == 4) {
// 					break;
