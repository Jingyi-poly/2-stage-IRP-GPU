...
void  OURemoteSolver::Solve() {
    ...
    switch (m_req.control_day_1) {
        ...
        default: {
            throw std::runtime_error("invalid control_day_1");
        }
    }
    if (traces) cout << "dp_ok!" << endl;

    // backtracking 
    // 将resize操作移到循环外部，避免重复调整vector大小
    ...
    for (int s = 0; s < m_req.nb_scenario; ++s) {
        ...
        while (t <= m_req.nb_days) {
            if (map_memory_.find({t, I, s}) == map_memory_.end()) {
                cout << "error: map_memory_ not found: (t, I, s) = (" << t << ", " << I << ", " << s << ")" << endl;
                cin.get();
            }
...
DayRes OURemoteSolver::SolveDayInventoryScenario(int day, double start_inventory, int scenario_id) {
    ...
    if (map_memory_.find({day, start_inventory, scenario_id}) != map_memory_.end()) {
        return map_memory_[{day, start_inventory, scenario_id}];
    }
