...
Genetic::Genetic(ParamsList *params_list, Population *population, clock_t ticks, bool traces, bool writeProgress) : params(params_list->general_params_), population(population), ticks(ticks), traces(traces), writeProgress(writeProgress)
{
	vector<int> tempVect;
	vector<vector<int>> chromTRand;
	vector<pattern> chromPRand;
...
//**为`rejeton`配置`localSearch`**：删除`rejeton`的`localSearch`成员，然后为其分配一个新的`LocalSearch`对象。
	...
	BatchRunFunc([&](int s) {
		...
		rejeton->sub_individus_[s]->localSearch = nullptr; // 设置为nullptr避免重复释放
...
int Genetic::crossPOX_OU() {
    ...
    for (int k = 1; k <= params->nbDays; k++) {
        for (int cus = params->nbDepots; cus < params->nbDepots + params->nbClients; cus++) {
            if (childDelivery[k][cus]) {  // 若布尔向量为 true，填满库存
	...
	for (int k = 0; k < params->nbDays; k++)
	{
		...
		if (k < j1 && !rejeton->chromT[day].empty())
		{
			...
			while (j != ((fin + 1) % rejeton->chromT[day].size()))
			{
				...
				garder.push_back(ii);
				rejeton->chromL[day][ii] = 1; //复制父代数据，如果父代数据为0，则子代数据也为0，否则为1
				hasBeenInserted[day][ii] = true;
				j = (j + 1) % rejeton->chromT[day].size();
			}
			rejeton->chromT[day].clear();
...
void Genetic::crossPOX_OU_day_1(Individu *child1, Individu *child2) {
	...
	// 将选中的sub-individual的数据同步到general_individu_
...
void Genetic::crossPOX_OU_ptr(Individu *child1, Individu *child2, Individu *general_individu = nullptr)
{
	...
	for (int k = 0; k < params->nbDays; k++)
	{
		...
		{
			debut = (int)(params->rng->genrand64_int64() % child1->chromT[day].size()); //定从第一个父代child1继承哪些数据。这可能是一个完整的序列、部分序列或没有数据。
			...
			while (j != ((fin + 1) % child1->chromT[day].size()))
			{
				...
				if(chromLParent1[day][ii] < 0.1) continue;
				garder.push_back(ii);
				child1->chromL[day][ii] = 1; //复制父代数据，如果父代数据为0，则子代数据也为0，否则为1
				hasBeenInserted[day][ii] = true;
	...
	// 	// 把第一天的数据同步为general_individu_
	// 	// cout << "已同步general_individu_到child1" << endl;
