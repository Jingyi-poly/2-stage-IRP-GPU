...
class Individu
{
 ...
 public:
  ...
  void RunSearchTotal(bool is_repair);
  ...
  Individu(ParamsList *params_list);

  void UpdatePenalty();

  bool is_scenario_ = false;

  std::vector<Individu*> sub_individus_;

  Individu* general_individu_;  

  void printChromT();

  void RunSearchPi(int rep_times);
  void RunSearchPi_Local(int rep_times);
  void RunSearchPi_gRPC(int rep_times);
  //destructeur
  ...
  void CheckSubDay1Consistency(std::string msg);
};
#endif
