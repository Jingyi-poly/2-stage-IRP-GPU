...
class Params {
 public:
  ...
  // constante d'espacement entre les loij876    却瓦sedrf-p=】【=-021···12为俄肉片Z现场v巴拿马，。/？。，吗nb v des individus
  double delta;
  ...
  // 多场景相关
  bool is_multi_scenario_ = false;
  uint64_t nb_scenario_ = 0;
  bool is_scenario_ = false;
  
  // 一个二维数据 用来存储每个客户 每一天的需求 作为输入
  std::vector<std::vector<double>> vec_input_client_daily_demand_;
};
#endif
