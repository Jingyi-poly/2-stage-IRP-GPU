...
// #include "irp/solver/Model_LotSizingPI.h"
...
// if (c.is_valid())
// {
// Nombre de ticks horloge que le programme est autorise a durer,Number of clock ticks allowed for program execution
// clock_t nb_ticks_allowed; 
// nb_ticks_allowed = CLOCKS_PER_SEC;
// 
// initialisation des Parametres a partir du fichier d'instance et du chemin InitializeParameter.From instance file and solution routes.
// de la solution This object is initialized through the Params class constructor, which receives multiple parameters.
//   
...
// 
// launch evolution StartEvolution
...
// const int BATCH_SIZE = 32; // Number of scenarios per batch
// vector<Population*> populations(paramsList.nb_scenarios_, nullptr);
//   
// cout << "Starting batch processing with " << BATCH_SIZE << " scenarios per batch..." << endl;
// cout << "Total scenarios: " << paramsList.nb_scenarios_ << endl;
//   
// Process scenarios in batches
// for (int batch_start = 0; batch_start < paramsList.nb_scenarios_; batch_start += BATCH_SIZE) {
// int batch_end = min(batch_start + BATCH_SIZE, (int)paramsList.nb_scenarios_);
// int current_batch_size = batch_end - batch_start;
//     
// cout << "Processing batch " << (batch_start / BATCH_SIZE + 1) 
// << " (scenarios " << batch_start << " to " << (batch_end - 1) << ")" << endl;
//     
// vector<thread> threads;
//     
// Create and start threads for current batch
// for (int s = batch_start; s < batch_end; s++) {
// threads.emplace_back([&paramsList, &c, &populations, s, nb_ticks_allowed]() {
// Params *mesParametres = paramsList.vec_params_[s];
//         
// Create independent output route for each scenario
// string scenario_solution_path = c.get_path_to_solution() + "_scenario_" + to_string(s);
// string scenario_bks_path = c.get_path_to_BKS() + "_scenario_" + to_string(s);
//         
// initial population 
// Population *population = new Population(mesParametres);
// populations[s] = population;
// 
//cout << "Scenario " << s << ": init solver" << endl;
//       
// on cree le solver we create the solver
//Create the solver
// Genetic solver(mesParametres, population, nb_ticks_allowed, true, true);
// 
// launch evolution StartEvolution
//cout << "######### Main descent phase for scenario " << s << " : "
// << mesParametres->nbVehiculesPerDep << endl;
//cout << " " << endl;
// 
//         
...
//cout << "Scenario " << s << ": solver running..." << endl;
// solver.evolve(max_iter, maxIterNonProd, 1);
//cout << scenario_solution_path << endl;
//cout << "Export Population for scenario " << s << " : " << endl;
// 
// population->ExportPop(scenario_solution_path, true);
//         
//cout << "Export BKS for scenario " << s << ":" << endl;
// population->ExportBKS(scenario_bks_path);
//       
// Note:Do not delete mesParametres here,because it is managed by ParamsList
// delete mesParametres; // Remove this line to avoid duplicate deletion
// });
// }
//     
...
// }
//     
// cout << "Batch " << (batch_start / BATCH_SIZE + 1) << " completed." << endl;
// }
//   
...
int mainIRP(commandline& c)
{
 ...
 if (c.is_valid())
 {
  // Time allowed for program execution(Using wall clock time, not CPU time)
  // Use command line parameter -t specified,Default is 1200 seconds(20minutes)
  ...
  ParamsList paramsList(c);
  if (traces) cout << "init population" << endl;
  ...
  // Create independent output route for each scenario
  // string scenario_solution_path = c.get_path_to_solution() + "_scenario_" + to_string(0) + ;
 
  Population *population = new Population(&paramsList);

  if (traces) cout << "create population" << endl;
  //Create the solver
  Genetic solver(&paramsList, population, nb_ticks_allowed, true, true);
  if (traces) cout << "create solver" << endl;
  
  int max_iter = 10000000;
  int maxIterNonProd = c.get_iter() != 0 ? c.get_iter() : 100;
  solver.evolve(max_iter, maxIterNonProd, 1);
  if (traces) cout << "evolve" << endl;
  population->ExportPop(c.get_path_to_solution(), true);
  // population->ExportBKS(scenario_bks_path);
  delete population;
  return 0;
...
// 
  // initialisation des Parametres a partir du fichier d'instance et du chemin
  // de la solution
//   Params *mesParametres = new Params(
//     c.get_path_to_instance(), c.get_path_to_solution(), c.get_type(),
...
//   vector<Insertion *> breakpoints = vector<Insertion *>(3);
  //The initial size of this vector is also3,This means it can store3pointers to Insertion objects.
//   double objective;
// 
...
int main(int argc, char *argv[])
{
 ...
 // Parse command line parameters to get thread count
 commandline c(argc, argv);
 ...
 if (c.get_num_threads() == 0) {
  // When thread count is 0, use gRPC mode for inventory optimization
  std::cout << "\n✓ Threading disabled for inventory optimization (threads=0) - using gRPC mode" << std::endl;
  g_thread_pool = nullptr; // Ensure thread pool is empty to trigger gRPC mode

  // But still create thread pool for routing optimization, auto-detect CPU cores
  size_t routing_threads = std::thread::hardware_concurrency();
  if (routing_threads == 0) {
   routing_threads = 4; // fallback: if detection fails, use 4 threads
  }
  std::cout << "Auto-detected " << routing_threads << " CPU cores" << std::endl;
  std::cout << "Initializing routing optimization with " << routing_threads << " threads" << std::endl;
  ...
  std::cout << "Local multi-threading mode (threads=" << num_threads << "):" << std::endl;
  std::cout << " - Routing optimization: auto-detected " << routing_threads << " CPU cores (parallel)" << std::endl;
  std::cout << " - Inventory optimization: " << num_threads << " threads (controlled by -threads parameter)" << std::endl;

  // Initialize routing optimization thread pool using all CPU cores
  initRoutingThreadPool(routing_threads);
  // Initialize inventory optimization thread pool with user-specified thread count
  initThreadPool(num_threads);
 } else {
  num_threads = std::thread::hardware_concurrency();
  std::cout << "Using default " << num_threads << " threads (hardware concurrency)" << std::endl;
 ...
 // Pass the parsed command line object to mainIRP
 mainIRP(c);

 // Clean up thread pool (only when initialized)
 if (c.get_num_threads() == 0) {
  // gRPC mode: clean up routing optimization thread pool
  destroyRoutingThreadPool();
 } else if (c.get_num_threads() > 0) {
  // Local multi-threading mode: clean up both thread pools
  destroyRoutingThreadPool(); // Clean up routing optimization thread pool
  destroyThreadPool();     // Clean up inventory optimization thread pool
 } else {
  // Default mode: clean up generic thread pool
  destroyThreadPool();
 }
 return 0;
}
