...
void Client::computeJourSuiv () 
{
	...
	// donnant computeJourSuiv[codeT][jourT] : peut on rajouter la tache assimilee 
	...
	// on remplace les 0 et les 1 par la donnee du prochain jour oa l'on peut
	// ajouter la tache

	for (int k=0 ; k <= (int)pow((float)2,(int)nbJours) ; k++)
	{
		// une petite marque dans le champ 0 qui etait vide a eta laissee
		// si cette valeur vaut -1 il n'y a rien a voir.
		if (jourSuiv[k][0] != -1)
		{

			// on parcourt de droite a gauche pour trouver le premier element non nul
			day = 0 ;
			j = (int)nbJours ;
			while ( day == 0 )
			{
