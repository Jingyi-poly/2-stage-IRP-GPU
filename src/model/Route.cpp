...
/*
forloopExecuteoncebooleanFlag.
place:represent/indicateNode.
charge:Demandor.
earlT:fromtoCurrentNodeTime.
Route:thisloopRoute,fromto.

noeud isCurrentinProcessNode.
place ,represent/indicatea/anNewNode.
charge add/plus,represent/indicatecumulativeDemand.
earlT add/plus,represent/indicatefromtoCurrentNodeTime.
foreachNode,UpdateinNodebeforeDemand chargeAvant andTime est.
UpdateRoutedata:inloopEndafter/back,UpdateRouteTime temps andDemand charge.

feasibleCheck:Functionbelow/undertoCheckRouteisnofeasible,that isisnobelowVehicleCapacity vehicleCapacity,andTimeisnobelowMaximumRouteTime maxRouteTime.ifFull,Routeforfeasible,elseforInfeasible.

InitializeInsert:finally,Functioncall initiateInsertions Method(MethodimplementinprovideGenerationin/middlenotcan/may,butcanisResetorprepareRoutesomeInsertdata).

to,thisFunctionistoUpdateRoutemainProperty,andCheckRoutefeasible.ifRouteanyPropertychange,addorDeleteCustomer,Functionpossiblywill becalltoalldataisNew.
*/
...
void Route::evalInsertClientp (Noeud * U) 
{
	...
	else // U is already in our route
	{
		...
		// temporarily we'll remove the node from the chainee list (in O(1))
		U->pred->suiv = U->suiv ;
		U->suiv->pred = U->pred ;
		courNoeud = depot ;

		// et parcourir la route a nouveau
		while (!courNoeud->estUnDepot || firstIt == true )
		{
			firstIt = false ;
			cout = params->timeCost[courNoeud->cour][U->cour] 
			...
			// au final on peut placer d'une meilleure maniere
			if (cout < bestInsertion[U->cour].detour - 0.0001)
			{ 
				bestInsertion[U->cour].detour = cout ;
				bestInsertion[U->cour].place = courNoeud ;
...
// pour un client donne, trouve la meilleure position d'insertion et sa quantite
/*
Customer(represent/indicateforNode Noeud * U)toinCurrentRoutein thebestInsertPositionandcorrespondinggoodsCount.FunctionandnotperformInsertoperation,onlyisandoutbestInsertPositionandcorrelationInfo.
Customer U notinCurrentRoute:
inbelow/under,FunctiontryoutwillCustomer U InsertCurrentRoutebestPosition.for,will traverseRoute,Calculatea/anpossiblyInsertCost,andwill itwithCurrentalreadybestInsertCostperformcompare.iftoa/anInsert,then/justUpdatebestInsertInfo.

Customer U alreadyinCurrentRoute:
first,FunctionCalculateCustomer U CurrentPositionCost,andwill itforbestInsertCost.
,fromin/middleremoveCustomer U,andtraverse entire againRoute,a/anInsertPosition.
inEndafter/back,Customer U NewInserttoin/middle.
*/
