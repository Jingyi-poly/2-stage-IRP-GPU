	...
	type(type), nbVehiculesPerDep(nbVeh)
{
	...
	debut = std::chrono::steady_clock::now(); // UseTime
	nbVehiculesPerDep = nbVeh;

	// ouverture du fichier en lecture
	fichier.open(nomInstance.c_str());

	// parsing des donneesopenFileprovideRead
	if (fichier.is_open())
	...
	// calcul des structures structureCalculate
	calculeStructures();
	...
	// InitializeDemandmatrix
	// init_scenario_2_daily_demand();
}
...
void Params::setMethodParams()
{
	...
	// parameters of the population //populationParameter
	el = 3;					// ***
	mu = 12;				// *
	lambda = 25;			// *
	...
	// parameters of the mutation	// change/becomeParameter
	...
	// parametres lies aux penalites adaptatives shouldParameter
...
// Parse data from the instance file
void Params::preleveDonnees(string nomInstance, int rou, bool stockout)
{
	// variables temporaires utilisees dans la fonction //TemporaryVariable
	vector<Client> cTemp;
	...
	if (type == 38) // IRP format of Archetti http://or-brescia.unibs.it/instances 
	{
		...
		{
			ordreVehicules.push_back(tempI);
			dayCapacity.push_back(0); //VehicleCapacity.xxxz
			nombreVehicules.push_back(nbDepots * nbVehiculesPerDep); //Depot*eachDepotVehicle
			for (int i = 0; i < nbDepots; i++)
				for (int j = 0; j < nbVehiculesPerDep; j++) //planningshouldUsewhichVehicle.eachInitialize
					ordreVehicules[kk].push_back(Vehicle(-1, -1, -1)); //specificDepotnumber/signal,MaximumrowTimeandVehicleCapacity.
		}
	...
	{
		// premiere ligne: description du probleme,problemDescription
...
// calcule les autres structures du programme Calculateprogramin theotherstructure
void Params::calculeStructures()
{
	...
	// Sort proximity list based on distance between customers/depots
	for (int i = nbDepots; i < nbClients + nbDepots; i++)
	{
		...
		// Based on prox (customer partition) and proxCst (fixed customer count)
		// Determine correlated customers and add to neighbor list
	...
	// on melange les prochestorandomtransformsommetsVoisinsListSequence.
	shuffleProches();
...
Client Params::getClient(int i,int rou)
{
	...
	else if (type == 38) // IRP format of Archetti et al.
	{
		...
		else //information of each customer
		{
			...
			// Adjusted:lowInitial inventory,firstRequiredDelivery
			// willInitial inventorylowtoMaximumInventory30%,ensurefirstDemandnooverwrite
			client.startingInventory = client.maxInventory * 0.30;

			std::vector<double>myVector(nbDays + 1);
			for(int i=1;i<nbDays + 1; ++i)	fichier >>myVector[i];
			myVector[0]= myVector[1],
			client.dailyDemand = myVector;
			//for(int i=0;i<nbDays + 1; ++i)	cout <<" = "<<client.dailyDemand[i];
...
Params::Params(Params *params, int decom, int *serieVisites, Vehicle **serieVehicles, int *affectDepots, int *affectPatterns, int depot, int jour, int nbVisites, int nbVeh)
{
	...
	// affectation des autres parametres Other parametersAssignment
	setMethodParams();
	...
	// calcul des distances CalculateDistance
	// on remplit les distances dans timeCost in timeCost fill inDistance
	for (int i = 0; i < nbClients + nbDepots; i++)
	{
...
void Params::decomposeRoutes(Params *params, int *serieVisites, Vehicle **serieVehicles, int nbVisites, int nbVeh)
{
	...
	// on place les donnees sur les vehicules
	...
	// ils ont toujours les anciens numeros
...
void Params::computeConstant_stockout()
{
	...
	if (isInventoryRouting)
	{
		...
		cout << inventoryCostSupplier <<endl;
		// cin.get();
	}
