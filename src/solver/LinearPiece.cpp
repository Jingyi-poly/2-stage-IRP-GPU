...
#include "irp/solver/LinearPiece.h"
#include <algorithm>
#include <iomanip>
...
double LinearPiece::cost(double x)
{
  ...
  if(eq(p1->x,p2->x)){
    if(eq(x,p1->x)){
      ...
      cerr <<"ERROR: LinearPiece cost: x="<<x<<" p1->x="<<p1->x<<" p2->x="<<p2->x<<" out of range"<<endl;
      return p1->y;
    }
  }
...
std::shared_ptr<LinearPiece> LinearPiece::getInBound(double lb, double ub)
{
  ...
  // debug: le(p2->x, ub)
    ...
    if (lt(p1->x, lb)){       // x1 < lb
      if (le(p2->x, ub)){     // x2 < ub
        lp = std::make_shared<LinearPiece>(lb, cost(lb), p2->x, p2->y);  // x2 <= ub 
      }
      else{
 lp = std::make_shared<LinearPiece>(lb, cost(lb), ub, cost(ub)); // x1 < lb < ub < x2 
      }
    }
    else{ // x1 >= lb
      double x = lb;
      ...
      if (le(p2->x, ub)){
        ...
        lp = std::make_shared<LinearPiece>(x, p1->y, p2->x, p2->y);  // x2 <= ub 
      }
      else {
...
LinearPiece::~LinearPiece()
{
  // delete p1;  // p1 is shared_ptr, no manual delete needed
  // delete p2;  // p2 is shared_ptr, no manual delete needed
