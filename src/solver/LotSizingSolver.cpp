...
#include <cmath>
#include <iomanip>
// Configure PLFunction vector F, and for each PLFunction set segment properties.
  ...
  : params(params), insertions(inst), client(client)
{
 ...
 // Use params, tmp, t, and client as constructor parameters. Store the shared_ptr to the new PLFunction object in F[t].
  ...
  if(params-> isstockout){
   ...
   for (int i = 0; i < F2[t]->nbPieces; i++){
     ...
     // : ";F2[0]->pieces[i]->fromInst->print();F2[0]->pieces[i]->print()}
   }
  ...
  std::shared_ptr<PLFunction> fromC, std::shared_ptr<PLFunction> fromF)
{
 ...
 for (int i = 0; i < fromC->nbPieces; i++){
  for (int j = 0; j < fromF->nbPieces; j++){
   ...
   if(trace) {
    // tmpc: ";fromPieceC->print();
    // tmpf: ";fromPieceF->print();
...
void LotSizingSolver::  solveEquationSystem_holding(std::shared_ptr<LinearPiece> C,
 ...
 //quantity = upperbound tolowerboundbetween iterger;
  if (ge(upperbound ,lowerbound) ){
    ...
    std::cerr << "ERROR: No integers between upperbound and lowerbound: " << upperbound<<" "<<lowerbound<<std::endl;
  }
  return;
 } 
...
bool LotSizingSolver::solve()
{
 ...
 // Create PLFunction vector for horizon (piecewise linear functions)
...
// Create Insertion vector (detour, load, place) shared pointers

 exceptionalPieces = vector<std::shared_ptr<LinearPiece>>(horizon);
 // Create LinearPiece vector (slope, p1, p2)

 for (int i = 0; i < horizon; i++)
 {
 exceptionalPieces[i] = std::make_shared<LinearPiece>(); // std::make_shared improves memory efficiency
 ...
 for (int t = 1; t < horizon; t++)
 {
  ...
  // calculate f_2
  auto f2_ptr = copyPLFunction(C[t - 1]);
...
bool LotSizingSolver::backtracking_stockout(){
 ...
 C[day]->getMinimalPiece_stockout(client, I[day], objective); //Iday:two x; minValue:two y
 ...
 while (tmp != nullptr){
  ...
  { 
   ...
   if(neq(I[day],0)){
    ...
    // Note: This indicates a potential numerical precision issue
   }
   //cout <<"day"<<day<<endl;
...
void LotSizingSolver::Firstday(vector<std::shared_ptr<PLFunction>> &C){//for the first day
 ...
 int piece_idx=0;
 // Istart+q-daily <=0 ->>>>>> q<=daily-Istart
 if(I<0){
...
bool LotSizingSolver::solve_stockout()
{
 ...
 C = vector<std::shared_ptr<PLFunction>>(horizon);
 // Create PLFunction vector for horizon
 ...
 breakpoints = vector<std::shared_ptr<Insertion>>(horizon);
// Create Insertion vector shared pointers

 exceptionalPieces = vector<std::shared_ptr<LinearPiece>>(horizon);
 // Create LinearPiece vector
...
{
    // shared_ptr will automatically clean up when going out of scope
    // No explicit delete needed for vector<shared_ptr<T>>
}
