...
#include "irp/solver/PLFunction.h"
#include <cmath>
#include <cassert>
...
PLFunction::PLFunction(Params *params, vector<Insertion> insertions, int day, int client) : params(params)
{
 ...
 // First, if this is the first insertion, set y and x values.
 // For other insertions, create linear segments based on detour and load values. Use calculateCost to compute costs.
 //addNewLinearPiecetoPLFunction:
    if (index == insertions.begin())
    {
      pre_x = 0;
      pre_y = index->detour;
      x = index->load;
 y = calculateCost(day, client, index->detour, x, index->load); //x,load: demand loadfree according toMaximum
...
PLFunction::PLFunction(Params *params, vector<Insertion> insertions, int day, int client,bool st) : params(params)
{
 ...
 //addNewLinearPiecetoPLFunction:
    if (index == insertions.begin())
    {
      ...
      x = index->load;
 y = calculateCost_holding(day, client, index->detour, x, index->load); //x,load: demand loadfree according toMaximum
...
PLFunction::PLFunction(Params *params, vector<Insertion> insertion, int day, int client, bool st, int daily) : params(params)
{
 ...
 //according toMaximum
      ...
      {  // make piecey
        ...
        x = pre_load + (index->detour - pre_detour) / params->penalityCapa;
         // detour = " << index->detour - pre_detour<< " capc "<< params->penalityCapa << endl;
...
void PLFunction::update_minValue(double & rep){
  ...
  if(lt(rep , 0)){
    cerr<<"ERROR: PLFunction update_minvalue: rep "<<rep+1e-4<<endl;
  }
...
std::shared_ptr<PLFunction> PLFunction::getInBound(double lb, double ub, bool updateValueAt0)
{
 ...
 plFunction->valueAt0 = this->pieces[0]->p2->y; //segment/pieceEndy
    }
  }
  plFunction->valueAt0 = this->valueAt0;
  ...
  for (int i = 0; i < nbPieces; i++){
    ...
    if (updateValueAt0 && lt(pieces[i]->p1->x, lb) && le(lb, pieces[i]->p2->x)  ) // lb is within this piece (inside)
    {
      ...
      plFunction->pieceAt0 = plFunction->copyPiece(pieces[i]);
...
double PLFunction::calculateCost(int day, int client, double detour, double demand, double freeload)
{
 ...
 // inventory cost first,CalculatespecificCustomerInventoryCostminus/reducetosupplierInventoryCost,thenmultiplytoNumber of days.
  
  double inventoryCost = (params->cli[client].inventoryCost  -  params->inventoryCostSupplier) 
                            * (double)(params->ancienNbDays - day);
 cost += inventoryCost * demand; //toinventoryCostthen multiplytodemandtoGetInventoryCost,thenadd/plustocoston/above.
...
std::shared_ptr<LinearPiece> PLFunction::copyPiece(std::shared_ptr<LinearPiece> source) {
  if (source == nullptr) return nullptr;
  auto dest = std::make_shared<LinearPiece>(source->p1->x, source->p1->y, source->p2->x, source->p2->y);
  dest->slope = source->slope;
  dest->fromC = source->fromC;
  dest->fromF = source->fromF;
  dest->fromInst = source->fromInst;
  dest->fromC_pre = source->fromC_pre;
  dest->replenishment_loss = source->replenishment_loss;
  return dest;
}
