...
int main(int argc, char *argv[])
{
    ...
    try
    {
        ...
        SplitCUDA splitCuda(params, 1);
        cout << "GPU SplitCUDA initialized (batch=1, scenarios=" << params.n_scenarios << ")" << endl;

        TourEvalFunc gpuEval = [&](const std::vector<int>& perm) -> double {
            Individual indiv(params, true);
            indiv.chromT.assign(perm.begin(), perm.end());
            std::vector<Individual*> batch = {&indiv};
            splitCuda.reset_batch(1);
            splitCuda.preprocess_batch(batch, params.nbVehicles);
            splitCuda.generate_split_batch(1);
            splitCuda.evaluateOnGPU_batch(batch);
            return indiv.eval.penalizedCost;
        };

        bool forceAllVisits = !commandline.ap.optionalVisit;
        cout << "Mode: " << (forceAllVisits ? "Benders MILP + GPU Split" : "PCTSP (optional visits)") << endl;

        PCTSPExact solver(params, pctspTimeLimit, commandline.verbose);
        PCTSPResult stochResult;

        if (forceAllVisits) {
            stochResult = solver.solveFullStochasticMILP(gpuEval);
        } else {
            cout << "\n========================================" << endl;
