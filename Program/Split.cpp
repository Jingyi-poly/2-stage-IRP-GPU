...
void Split::generalSplit(Individual & indiv, int nbMaxVehicles, int idx_scen)
{
...
int Split::splitSimple(Individual & indiv, int idx_scen)
{
	...
	else
	{
		...
		for (int i = 1; i <= params.nbClients; i++)
		{
			...
			if (debug) printf("Scenario %d: data[%d]--> %f\n",idx_scen,i,potential[0][i]);
			...
			if (debug) printf("      front: %d   back: %d\n", queue.indexFront, queue.indexBack);

			if (i < params.nbClients)
			{
				// If i is not dominated by the last of the pile
				if (debug) printf("      dominates? %d   \n", dominates(queue.get_back(), i, 0));
				...
				if (!dominates(queue.get_back(), i, 0))
				{
					...
					while (queue.size() > 0 && dominatesRight(queue.get_back(), i, 0)){
						if (debug) printf("      dominatesRight? %d...... size %d\n", dominatesRight(queue.get_back(), i, 0),queue.size());
						queue.pop_back();
					}
					if (debug) printf("      dominatesRight? %d...... size %d\n", dominatesRight(queue.get_back(), i, 0),queue.size());
					queue.push_back(i);
				}
				if (debug) printf("  out  front: %d   back: %d\n", queue.indexFront, queue.indexBack);
				// Check iteratively if front is dominated by the next front
				if (debug) printf("      prop 1 = %f\n", propagate(queue.get_front(), i + 1, 0));
				if (debug) printf("      prop 2 = %f, next front: %d\n\n", propagate(queue.get_next_front(), i + 1, 0), queue.get_next_front());
				// for (int q = 0; q < queue.size(); ++q){
				// 	std::cout<<queue.myDeque[q]<<"\n";
				// }
				// int g; std::cin>>g;
	...
	// int end = generateChromR(indiv, idx_scen);

	// std::cout<<"!!!!!!!!!! large    "<<idx_scen<<"\n";
...
int Split::splitLF(Individual & indiv, int idx_scen)
{
...
std::vector < std::vector <int>> Split::generalSplitReturn(Individual & indiv, int nbMaxVehicles, int idx_scen)
{
	// Do not apply Split with fewer vehicles than the trivial (LP) bin packing bound
	maxVehicles = std::max<int>(nbMaxVehicles, std::ceil(params.totalDemand/params.vehicleCapacity));
	...
	// We first try the simple split, and then the Split with limited fleet if this is not successful
	std::vector < std::vector <int>> chromR_res(params.nbVehicles);
	if (splitSimpleReturn(indiv, idx_scen, chromR_res) == 0) {
		chromR_res = std::vector < std::vector <int>>(params.nbVehicles);
		splitLFReturn(indiv, idx_scen, chromR_res);
		std::cout<<"Used extra step for idx: "<<idx_scen<<"\n";
	}
...
int Split::splitSimpleReturn(Individual & indiv, int idx_scen, std::vector < std::vector <int> > & chromR_res)
{
	// Reinitialize the potential structures
	potential[0][0] = 0;
	for (int i = 1; i <= params.nbClients; i++)
	...
	for (int k = maxVehicles - 1; k >= 0; k--)
	{
		...
		int begin = pred[0][end];
		for (int ii = begin; ii < end; ii++){
			chromR_res[k].push_back(indiv.chromT[ii]);
		}
...
// Split for problems with limited fleet
int Split::splitLFReturn(Individual & indiv, int idx_scen, std::vector < std::vector <int> > & chromR_res)
{
	// Initialize the potential structures
	potential[0][0] = 0;
	for (int k = 0; k <= maxVehicles; k++)
