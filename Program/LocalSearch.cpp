...
void LocalSearch::setLocalVariablesRouteU()
{
	...
	// loadU_scen = params.cli[nodeUIndex].demands_scenarios;
	// loadX_scen = params.cli[nodeXIndex].demands_scenarios;
	loadU_scen.resize(params.n_scenarios);
	loadX_scen.resize(params.n_scenarios);
	for (int i = 0; i < params.n_scenarios; ++i){
		loadU_scen[i] = params.cli[nodeUIndex].demands_scenarios[i];
		loadX_scen[i] = params.cli[nodeXIndex].demands_scenarios[i];
	}
...
void LocalSearch::setLocalVariablesRouteV()
{
	...
	// loadV_scen = params.cli[nodeVIndex].demands_scenarios;
	// loadY	 = params.cli[nodeYIndex].demand;
	// loadY_scen = params.cli[nodeYIndex].demands_scenarios;
	loadV_scen.resize(params.n_scenarios);
	loadY_scen.resize(params.n_scenarios);
	for (int i = 0; i < params.n_scenarios; ++i){
		loadV_scen[i] = params.cli[nodeVIndex].demands_scenarios[i];
		loadY_scen[i] = params.cli[nodeYIndex].demands_scenarios[i];
	}
...
bool LocalSearch::move1()
{
	...
	if (!intraRouteMove)
	{
		...
		for (int i = 0; i < params.n_scenarios; ++i){
			costSuppU += penaltyExcessLoad(routeU->load_scen[i] - loadU_scen[i]);
			costSuppV += penaltyExcessLoad(routeV->load_scen[i] + loadU_scen[i]);
		}
	}
	// std::cout<<"costSuppU: "<<costSuppU<<"   costSuppV: "<<costSuppV<<"\n";
	// std::cout<<params.n_scenarios<<"   costSuppV: "<<params.cli[0].demands_scenarios.size()<<"\n";
...
bool LocalSearch::move2()
{
	...
	if (!intraRouteMove)
	{
		...
		for (int i = 0; i < params.n_scenarios; ++i){
			costSuppU += penaltyExcessLoad(routeU->load_scen[i] - loadU_scen[i] - loadX_scen[i]);
			costSuppV += penaltyExcessLoad(routeV->load_scen[i] + loadU_scen[i] + loadX_scen[i]);
		}
	}
...
bool LocalSearch::move4()
{
	...
	if (!intraRouteMove)
	{
		...
		for (int i = 0; i < params.n_scenarios; ++i){
			costSuppU += penaltyExcessLoad(routeU->load_scen[i] + loadV_scen[i] - loadU_scen[i]);
			costSuppV += penaltyExcessLoad(routeV->load_scen[i] + loadU_scen[i] - loadV_scen[i]);
		}
	}
...
bool LocalSearch::move5()
{
	...
	if (!intraRouteMove)
	{
		...
		for (int i = 0; i < params.n_scenarios; ++i){
			costSuppU += penaltyExcessLoad(routeU->load_scen[i] + loadV_scen[i] - loadU_scen[i] - loadX_scen[i]);
			costSuppV += penaltyExcessLoad(routeV->load_scen[i] + loadU_scen[i] + loadX_scen[i] - loadV_scen[i]);
		}
	}
...
bool LocalSearch::move6()
{
	...
	if (!intraRouteMove)
	{
		...
		for (int i = 0; i < params.n_scenarios; ++i){
			costSuppU += penaltyExcessLoad(routeU->load_scen[i] + loadV_scen[i] + loadY_scen[i] - loadU_scen[i] - loadX_scen[i]);
			costSuppV += penaltyExcessLoad(routeV->load_scen[i] + loadU_scen[i] + loadX_scen[i] - loadV_scen[i] - loadY_scen[i]);
		}
	}
...
bool LocalSearch::move8()
{
	...
	for (int i = 0; i < params.n_scenarios; ++i){
		cost += penaltyExcessLoad(nodeU->cumulatedLoad_scen[i] + nodeV->cumulatedLoad_scen[i]);
		cost += penaltyExcessLoad(routeU->load_scen[i] + routeV->load_scen[i] - nodeU->cumulatedLoad_scen[i] - nodeV->cumulatedLoad_scen[i]);
	}
...
bool LocalSearch::move9()
{
	...
	for (int i = 0; i < params.n_scenarios; ++i){
		cost += penaltyExcessLoad(nodeU->cumulatedLoad_scen[i] + routeV->load_scen[i] - nodeV->cumulatedLoad_scen[i]);
		cost += penaltyExcessLoad(nodeV->cumulatedLoad_scen[i] + routeU->load_scen[i] - nodeU->cumulatedLoad_scen[i]);
	}
...
bool LocalSearch::swapStar()
{
	...
	for (nodeU = routeU->depot->next; !nodeU->isDepot; nodeU = nodeU->next)
	{
		for (nodeV = routeV->depot->next; !nodeV->isDepot; nodeV = nodeV->next)
		{
			...
			double deltaPenRouteU = 0.;
			double deltaPenRouteV = 0.;

			for (int i = 0; i < params.n_scenarios; ++i){
				deltaPenRouteU = penaltyExcessLoad(routeU->load_scen[i] + params.cli[nodeV->cour].demands_scenarios[i] - params.cli[nodeU->cour].demands_scenarios[i]);
				deltaPenRouteV = penaltyExcessLoad(routeV->load_scen[i] + params.cli[nodeU->cour].demands_scenarios[i] - params.cli[nodeV->cour].demands_scenarios[i]);
			}
			deltaPenRouteU -= routeU->penalty;
			deltaPenRouteV -= routeV->penalty;
	...
	for (nodeU = routeU->depot->next; !nodeU->isDepot; nodeU = nodeU->next)
	{
		...
		for (int i = 0; i < params.n_scenarios; ++i){
			mySwapStar.moveCost += penaltyExcessLoad(routeU->load_scen[i] - params.cli[routeU->cour].demands_scenarios[i]);
			mySwapStar.moveCost += penaltyExcessLoad(routeV->load_scen[i] + params.cli[routeU->cour].demands_scenarios[i]);
		}
		mySwapStar.moveCost -= routeU->penalty;
		mySwapStar.moveCost -= routeV->penalty;
	...
	for (nodeV = routeV->depot->next; !nodeV->isDepot; nodeV = nodeV->next)
	{
		...
		// mySwapStar.moveCost += penaltyExcessLoad(routeU->load + params.cli[nodeV->cour].demand) - routeU->penalty;
		// mySwapStar.moveCost += penaltyExcessLoad(routeV->load - params.cli[nodeV->cour].demand) - routeV->penalty;
		
		for (int i = 0; i < params.n_scenarios; ++i){
			mySwapStar.moveCost += penaltyExcessLoad(routeU->load_scen[i] + params.cli[nodeV->cour].demands_scenarios[i]);
			mySwapStar.moveCost += penaltyExcessLoad(routeV->load_scen[i] - params.cli[nodeV->cour].demands_scenarios[i]);
		}
		mySwapStar.moveCost -= routeU->penalty;
		mySwapStar.moveCost -= routeV->penalty;
...
void LocalSearch::updateRouteData(Route * myRoute)
{
	int myplace = 0;
	// double myload = 0.;
	std::vector<double> myload_scen;
	for (int i = 0; i < params.n_scenarios; ++i){
		myload_scen.push_back(0.);
	}
	double mytime = 0.;
	...
	mynode->cumulatedLoad_scen.resize(params.n_scenarios);
	for (int i = 0; i < params.n_scenarios; ++i){
		mynode->cumulatedLoad_scen[i] = 0.;
	}
	...
	while (!mynode->isDepot || firstIt)
	{
		...
		mynode->cumulatedLoad_scen.resize(params.n_scenarios);
		for (int i = 0; i < params.n_scenarios; ++i){
			myload_scen[i] += params.cli[mynode->cour].demands_scenarios[i];
			mynode->cumulatedLoad_scen[i] = myload_scen[i];
		}
	...
	// myRoute->load = myload;
	myRoute->load_scen = myload_scen;
	myRoute->penalty = penaltyExcessDuration(mytime);
	// myRoute->penalty += penaltyExcessLoad(myload);
	for (int i = 0; i < params.n_scenarios; ++i){
		myRoute->penalty += penaltyExcessLoad(myload_scen[i]);
	}
